...
function startGame() {
    ...
    // Attendre un peu puis cacher la bille et renverser les tasses
    setTimeout(() => {
        hideBall();
        characterMessage.textContent = 'Je vais maintenant renverser les tasses !';
        setTimeout(() => {
            flipCups();
        }, 1000);
    }, 2000);
...
function flipCups() {
    ...
    characterMessage.textContent = 'Je renverse les tasses !';
    
    // Désactiver les tasses pendant le retournement
    cups.forEach(cup => cup.classList.add('disabled'));

    // Animer le retournement de chaque tasse avec la baguette
    cups.forEach((cup, index) => {
        setTimeout(() => {
            // Animer la baguette
            if (wand) {
                wand.style.transition = 'transform 0.4s cubic-bezier(0.68, -0.55, 0.265, 1.55)';
                wand.style.transform = 'rotate(-45deg)';
            ...
            // Ajouter un son/animation de retournement
            cup.style.transition = 'transform 0.4s cubic-bezier(0.68, -0.55, 0.265, 1.55)';
            cup.style.transform = 'scaleY(0.1) rotateX(90deg)';
            
            // Retourner à la position normale
            setTimeout(() => {
                cup.style.transition = 'transform 0.4s cubic-bezier(0.68, -0.55, 0.265, 1.55)';
                cup.style.transform = 'scaleY(1) rotateX(0deg)';
            }, 200);
        }, index * 200); // Décalage entre chaque tasse
    });

    // Après le retournement, commencer le mélange
    setTimeout(() => {
        characterMessage.textContent = 'Maintenant je vais les mélanger !';
        mixCups();
    }, 1200);
}
